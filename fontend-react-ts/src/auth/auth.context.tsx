...
const AuthContextProvider = ({ children }: Iprops) => {
  ...
  const initializeAuthContext = useCallback(async () => {
    try {
      ...
      if (token) {
        ...
        // In response, we receive jwt token and use data
        const { newToken, userInfo } = response.data;
        setSession(newToken);
        //Show me error Cannot find name 'dispatch'.
        dispatch({
  ...
  //In start of Application ,We call intiializeAuthContext to be suere about authenticaton status

  useEffect(() => {
    console.log("AuthContext Initialization start");
    initializeAuthContext()
      .then(() => console.log("initializeAuthContxt was successfull"))
  ...
  //We create an object for values of contex provider
  //This will keep our codes more readable

  const valuesObject:IAuthContext = {
